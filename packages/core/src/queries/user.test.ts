...
describe('user query', () => {
  ...
  it('countUsers', async () => {
    ...
    const expectSql = sql`
      select count(*)
      from ${table}
      where ${table}.${fields.primaryEmail} ilike $1 or ${table}.${fields.primaryPhone} ilike $2 or ${table}.${fields.username} ilike $3 or ${table}.${fields.name} ilike $4
    `;
  ...
  it('countUsers with hideAdminUser', async () => {
    ...
    const expectSql = sql`
      select count(*)
      from ${table}
      left join ${usersRolesTable} ON ${table}.${fields.id} = ${usersRolesTable}.${usersRolesFields.userId}
      left join ${rolesTable} ON ${rolesTable}.${rolesFields.id} = ${usersRolesTable}.${usersRolesFields.roleId}
      where ${rolesTable}.${rolesFields.name} !== ${UserRole.Admin}
      and (${table}.${fields.primaryEmail} ilike $2 or ${table}.${fields.primaryPhone} ilike $3 or ${table}.${fields.username} ilike $4 or ${table}.${fields.name} ilike $5)
    `;
  ...
  it('countUsers with isCaseSensitive', async () => {
    ...
    const expectSql = sql`
      select count(*)
      from ${table}
      where ${table}.${fields.primaryEmail} like $1 or ${table}.${fields.primaryPhone} like $2 or ${table}.${fields.username} like $3 or ${table}.${fields.name} like $4
    `;
